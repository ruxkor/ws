...
WebSocket.prototype.pause = function() {
  if (this.readyState != WebSocket.OPEN) return this.emit('error', new Error('not opened'));
 ...
 * @param {boolean} dontFailWhenClosed indicates whether or not to emit an error if the connection isnt open
 * @api public
 */
...
WebSocket.prototype.ping = function(data, options, dontFailWhenClosed) {
  if (this.readyState != WebSocket.OPEN) {
    ...
    return this.emit('error', new Error('not opened'));
  }
  options = options || {};
...
WebSocket.prototype.send = function(data, options, cb) {
  ...
  if (this.readyState != WebSocket.OPEN) {
    ...
    else this.emit('error', new Error('not opened'));
    return;
  }
  if (!data) data = '';
  if (this._queue) {
  ...
  if (data instanceof fs.ReadStream) {
    ...
    sendStream(this, data, options, function(error) {
      ...
      else if (error) return self.emit('error', error);
    });
  }
...
WebSocket.prototype.stream = function(options, cb) {
  ...
  if (typeof cb != 'function') return this.emit('error', new Error('callback must be provided'));
  if (this.readyState != WebSocket.OPEN) {
    if (typeof cb == 'function') cb(new Error('not opened'));
    else this.emit('error', new Error('not opened'));
    return;
  ...
  var send = function(data, final) {
    try {
      if (self.readyState != WebSocket.OPEN) return this.emit('error', new Error('not opened'));
...
function initAsClient(address, options) {
  ...
  if (options.value.pfx || options.value.key || options.value.passphrase || options.value.cert || options.value.ca || options.value.ciphers || options.value.rejectUnauthorized) {
    if (isNodeV4) {
